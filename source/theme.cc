...
Theme::Theme(OperationMode mode)
    : AbstractOOXmlFile { mode }
{
}

const char* kDefaultTheme = "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>\n"
...
void Theme::ComposeXml(QIODevice* device) const { device->write(kDefaultTheme); }

bool Theme::ParseXml(QIODevice* device)
{
    Q_UNUSED(device)
    return true;
}
